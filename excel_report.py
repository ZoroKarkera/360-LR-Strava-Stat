...
def write_heatmap(ws, heatmap, week_start):
    ...
    if not heatmap:
        write_empty_state(ws, 11, 6, "No runs logged this week yet.")
        return

    start_row = 11
    for row_offset, runner in enumerate(sorted(heatmap.keys())):
        ...
        for day_index, day in enumerate(DAYS, start=7):
            ...
            total += distance
            ws.cell(row=row, column=day_index).value = distance
            ws.cell(row=row, column=day_index).number_format = "0.0"

        ws.cell(row=row, column=14).value = round(total, 1)
        ws.cell(row=row, column=14).number_format = '0.0 "km"'
        style_body_row(ws, row, 6, 14)

    last_row = start_row + len(heatmap) - 1
